...
section "install"
        ...
        MessageBox MB_YESNO|MB_ICONQUESTION "$(VCRedistNotInstalled)" IDYES install_vcredist IDNO skip_vcredist
        
        install_vcredist:
            DetailPrint "$(VCRedistDownloading)"
            NSISdl::download /TIMEOUT=30000 "${VCREDIST_URL}" "$PLUGINSDIR\vcredist.exe"
            Pop $1
            ${If} $1 == "success"
