    ...
    : QObject(parent)
{
    if (isHelper) {
        ...
        fopen_s(&m_debugFile, debugFileName.toStdString().c_str(), "w");
    }
    ...
    res = m_IWbemLocator->ConnectServer(_bstr_t(L"ROOT\\Microsoft\\Windows\\Storage"), NULL, NULL, NULL, 0, NULL, NULL, &m_IWbemServices);
    if (FAILED(res)) {
        _com_error err(res);
...
QMap<quint32, QString> WinDiskManagement::getUSBDeviceList()
{
    ...
    // BusType = 7 = USB
    res = m_IWbemServices->ExecQuery(_bstr_t(L"WQL"), _bstr_t(L"SELECT * FROM MSFT_Disk WHERE BusType = 7"), WBEM_FLAG_RETURN_IMMEDIATELY, NULL, &pEnumDiskObjects);
    if (FAILED(res)) {
        _com_error err(res);
        logMessage(QtWarningMsg, QStringLiteral("WMI query failed. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
        return result;
    }
    ...
    while (true) {
        ...
        if ((pDiskObject->Get(_bstr_t(L"Number"), 0, &var, 0, 0)) == WBEM_S_NO_ERROR) {
            if (var.vt == VT_I4) {
                index = var.intVal;
                logMessage(QtDebugMsg, QStringLiteral("Disk Index: %1").arg(index));
        ...
        if ((pDiskObject->Get(_bstr_t(L"__PATH"), 0, &var, 0, 0)) == WBEM_S_NO_ERROR) {
            if (var.vt == VT_BSTR) {
                ...
                logMessage(QtDebugMsg, QStringLiteral("Device ID%1").arg(deviceID));
            }
            VariantClear(&var);
        }
        pDiskObject->Release();
...
QMap<quint32, bool> WinDiskManagement::getDevicePartitions(quint32 index)
{
    ...
    if (FAILED(res)) {
        _com_error err(res);
        logMessage(QtWarningMsg, QStringLiteral("Query for disk partitions failed. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
        return result;
    }
    ...
    while (true) {
        ...
        if ((pPartitionObject->Get(_bstr_t(L"PartitionNumber"), 0, &var, 0, 0)) == WBEM_S_NO_ERROR) {
            if (var.vt == VT_I4) {
                ...
                logMessage(QtDebugMsg, QStringLiteral("Partition Index: %1").arg(partitionIndex));
            } else if (var.vt == VT_UI4) {
        ...
        if ((pPartitionObject->Get(_bstr_t(L"__PATH"), 0, &var, 0, 0)) == WBEM_S_NO_ERROR) {
            if (var.vt == VT_BSTR) {
                ...
                logMessage(QtDebugMsg, QStringLiteral("Partition path: %1").arg(partitionPath));
            }
            VariantClear(&var);
        }
        ...
        if ((pPartitionObject->Get(_bstr_t(L"DriveLetter"), 0, &var, 0, 0)) == WBEM_S_NO_ERROR) {
            if (var.vt == VT_I2) {
                wchar_t driveLetter = static_cast<wchar_t>(var.iVal);
                mountable = driveLetter != L'\0';
                if (mountable) {
                    logMessage(QtDebugMsg, QStringLiteral("DriveLetter: %1. Partition is mountable").arg(QString::fromWCharArray(&driveLetter, 1)));
                } else {
                    logMessage(QtDebugMsg, QStringLiteral("No assigned letter. Partition is not mountable"));
                }
            }
            VariantClear(&var);
        }
...
bool WinDiskManagement::clearPartitions(qint32 index)
{
    ...
    while (true) {
        ...
        if (SUCCEEDED(res)) {
            ...
            res = m_IWbemServices->ExecMethod(_bstr_t(partitionPath.toStdWString().c_str()), _bstr_t(L"DeleteObject"), 0, NULL, NULL, &pOutParams, NULL);
            if (FAILED(res)) {
                _com_error err(res);
                logMessage(QtWarningMsg, QStringLiteral("Failed to delete partition. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
                return false;
            }
            ...
            if (pOutParams) {
                ...
                if (returnValueVar.vt == VT_I4 && returnValueVar.intVal != 0) {
                    logMessage(QtWarningMsg, QStringLiteral("Failed to delete partition. Error code: %1").arg(QString::number(returnValueVar.intVal, 16)));
...
bool WinDiskManagement::formatPartition(const QChar &driveLetter)
{
    ...
    if (FAILED(res)) {
        _com_error err(res);
        logMessage(QtWarningMsg, QStringLiteral("Query for MSFT_Volume failed. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
        return false;
    }
    ...
    while (true) {
        ...
        res = m_IWbemServices->GetObject(_bstr_t(L"MSFT_Volume"), 0, NULL, &pClass, NULL);
        if (FAILED(res)) {
            _com_error err(res);
            logMessage(QtWarningMsg, QStringLiteral("WMI query to get MSFT_Volume object failed. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
            return false;
        }
        ...
        if (FAILED(res)) {
            _com_error err(res);
            logMessage(QtWarningMsg, QStringLiteral("WMI query to get 'Format' method on MSFT_Volume object failed. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
            return false;
        }
        ...
        if (FAILED(res)) {
            _com_error err(res);
            logMessage(QtWarningMsg, QStringLiteral("WMI spawn instance failed. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
            return false;
        }
        ...
        if (FAILED(res)) {
            _com_error err(res);
            logMessage(QtWarningMsg, QStringLiteral("Failed to set 'FileSystem' parameter. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
            return false;
        }
        ...
        if (FAILED(res)) {
            logMessage(QtWarningMsg, QStringLiteral("Failed to set 'Full' parameter."));
            return false;
        }

        volumeFound = true;
        res = m_IWbemServices->ExecMethod(_bstr_t(volumePath.toStdString().c_str()), _bstr_t(L"Format"), 0, NULL, pInParams, &pOutParams, NULL);
        if (FAILED(res)) {
            _com_error err(res);
            logMessage(QtWarningMsg, QStringLiteral("Failed to format the volume. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
        }
        ...
        if (pOutParams) {
            ...
            pOutParams->Get(L"ReturnValue", 0, &returnValueVar, NULL, NULL);
            if (returnValueVar.vt == VT_I4 && returnValueVar.intVal != 0) {
                logMessage(QtWarningMsg, QStringLiteral("Failed to format the volume. Error code: %1").arg(QString::number(returnValueVar.intVal, 16)));
                VariantClear(&returnValueVar);
                return false;
            } else {
...
bool WinDiskManagement::refreshDiskDrive(const QString &diskPath)
{
    ...
    res = m_IWbemServices->ExecMethod(_bstr_t(diskPath.toStdWString().c_str()), _bstr_t(L"Refresh"), 0, NULL, NULL, &pOutParams, NULL);
    ...
    if (FAILED(res)) {
        _com_error err(res);
        logMessage(QtWarningMsg, QStringLiteral("Failed to refresh the disk. Error = %1").arg(QString::fromWCharArray(err.ErrorMessage())));
        return false;
    }
...
bool WinDiskManagement::unmountVolumes(quint32 index)
{
    ...
    for (char i = 0; i < 26; i++) {
        if (drives & (1 << i)) {
            ...
            logMessage(QtWarningMsg, QStringLiteral("Checking drive: %1").arg(drivePath));
                ...
                for (uint j = 0; j < vde.NumberOfDiskExtents; j++) {
                    if (vde.Extents[j].DiskNumber == index) {
                        ...
                        if (!DeleteVolumeMountPointA(volumePath.toStdString().c_str())) {
                            logMessage(QtWarningMsg, QStringLiteral("Couldn't remove the drive: %1").arg(getLastError()));
                            return false;
                        }
                        logMessage(QtDebugMsg, QStringLiteral("Successfully unmounted volume: %1").arg(currentDrive));
                        break;
                    }
                }
            }
...
QChar WinDiskManagement::mountVolume(const QString &logicalName)
{
    ...
    if (!driveSize || driveSize > sizeof(drives)) {
        logMessage(QtWarningMsg, QStringLiteral("Failed to get drive letter mountpoint: %1").arg(getLastError()));
        return QChar();
    }
    ...
    if (!::SetVolumeMountPointA(drivePath.c_str(), logicalName.toStdString().c_str())) {
        logMessage(QtWarningMsg, QStringLiteral("Couldn't mount %1 as %2: %3").arg(logicalName).arg(driveLetter).arg(getLastError()));
        return QChar();
    }

    logMessage(QtDebugMsg, QStringLiteral("Successfuly mounted %1 as %2").arg(logicalName).arg(driveLetter));
    return QChar(driveLetter);
}

bool WinDiskManagement::writeFileWithRetry(HANDLE driveHandle, LPCVOID buffer, WORD numberOfBytesToWrite, int numberOfRetries)
{
    bool success = false;
    ...
    if (!readFilePointer) {
        logMessage(QtWarningMsg, QStringLiteral("Could not read file pointer: %1").arg(getLastError()));
    }

    for (int attempts = 1; attempts <= numberOfRetries; attempts++) {
        if ((attempts > 1) && (!SetFilePointerEx(driveHandle, filePointer, NULL, FILE_BEGIN))) {
            logMessage(QtWarningMsg, QStringLiteral("Could not set file pointer"));
            break;
        }
        if (WriteFile(driveHandle, buffer, numberOfBytesToWrite, &writtenBytes, NULL)) {
            if (numberOfBytesToWrite == writtenBytes) {
                return true;
            }
        } else {
            logMessage(QtWarningMsg, QStringLiteral("Failed to write data to the drive"));
            success = false;
        }
...
bool WinDiskManagement::clearPartitionTable(HANDLE driveHandle, quint64 driveSize, quint32 sectorSize)
{
    ...
    uint8_t *zeroBuffer = static_cast<uint8_t *>(calloc(sectorSize, sectorsToClear));
    if (!zeroBuffer) {
        logMessage(QtWarningMsg, QStringLiteral("Couldn't allocate zero buffer"));
        return false;
    }

    filePointer.QuadPart = 0ULL;
    if (!SetFilePointerEx(driveHandle, filePointer, &filePointer, FILE_BEGIN) || (filePointer.QuadPart != 0ULL)) {
        logMessage(QtWarningMsg, QStringLiteral("Couldn't reset disk position: %1").arg(getLastError()));
    }

    if (!writeFileWithRetry(driveHandle, zeroBuffer, static_cast<DWORD>(sectorSize * sectorsToClear), 4)) {
        logMessage(QtWarningMsg, QStringLiteral("Couldn't write zero data to the drive: %1").arg(getLastError()));
    }
...
bool WinDiskManagement::clearDiskDrive(HANDLE driveHandle)
{
    ...
    if (!ret) {
        logMessage(QtWarningMsg, QStringLiteral("Couldn't delete drive layout"));
        return false;
    }
...
bool WinDiskManagement::createGPTPartition(HANDLE driveHandle, quint64 diskSize, quint32 sectorSize)
{
    ...
    ret = DeviceIoControl(driveHandle, IOCTL_DISK_CREATE_DISK, &createDisk, sizeof(createDisk), NULL, 0, NULL, NULL);
    if (!ret) {
        logMessage(QtWarningMsg, QStringLiteral("Failed to create GPT partition table on the disk."));
        return false;
    }

    ret = DeviceIoControl(driveHandle, IOCTL_DISK_UPDATE_PROPERTIES, NULL, 0, NULL, 0, NULL, NULL);
    if (!ret) {
        logMessage(QtWarningMsg, QStringLiteral("Couldn't update disk properties."));
        return false;
    }
    ...
    if (!ret) {
        logMessage(QtWarningMsg, QStringLiteral("Failed to set GPT partition layout on the disk."));
        return false;
    }
...
QString WinDiskManagement::getLogicalName(quint32 index)
{
    QString result;
    ...
    for (int i = 0; drive == INVALID_HANDLE_VALUE; i++) {
        if (i == 0) {
            ...
            if (!FindNextVolumeA(volume, volumeName, sizeof(volumeName))) {
                logMessage(QtWarningMsg, QStringLiteral("Couldn't access next GUID volume: %1").arg(getLastError()));
                break;
            }
        }
