...
QMap<quint32, QString> WinDiskManagement::getUSBDeviceList()
{
    ...
    // BusType = 7 = USB | BusType = 12 = SD
    res = m_IWbemServices->ExecQuery(_bstr_t(L"WQL"), _bstr_t(L"SELECT * FROM MSFT_Disk WHERE BusType = 7 OR BusType = 12"), WBEM_FLAG_RETURN_IMMEDIATELY, NULL, &pEnumDiskObjects);
    if (FAILED(res)) {
        _com_error err(res);
    ...
    while (true) {
        ...
        if (index > 0) {
            logMessage(QtDebugMsg, QStringLiteral("Found device with index %1").arg(index));
...
bool WinDiskManagement::writeFileWithRetry(HANDLE driveHandle, void *buffer, qint64 numberOfBytesToWrite, int numberOfRetries)
{
    bool success = false;
    ...
    for (int attempts = 1; attempts <= numberOfRetries; attempts++) {
        if ((attempts > 1) && (!SetFilePointerEx(driveHandle, filePointer, NULL, FILE_BEGIN))) {
            logMessage(QtCriticalMsg, QStringLiteral("Could not set file pointer"));
            break;
        }
...
bool WinDiskManagement::clearPartitionTable(HANDLE driveHandle, quint64 driveSize, quint32 sectorSize)
{
    ...
    if (!writeFileWithRetry(driveHandle, zeroBuffer, sectorSize * sectorsToClear, 4)) {
        logMessage(QtCriticalMsg, QStringLiteral("Couldn't write zero data to the drive: %1").arg(getLastError()));
    }
