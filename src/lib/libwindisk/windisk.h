...
public:
    ...
    // Unmounts all logical volumes
    bool unmountVolumes(quint32 index);
    ...
    QString getLogicalName(quint32 index);
    // Refreshes the partition layout
    bool refreshPartitionLayout(HANDLE driveHandle);

private:
    bool writeFileWithRetry(HANDLE driveHandle, LPCVOID buffer, WORD numberOfBytesToWrite, int numberOfRetries);
