...
bool zeroDrive(const std::wstring &drivePath)
{
    HANDLE hDrive = CreateFileW(drivePath.c_str(), // Drive path (e.g., \\.\PhysicalDrive1)
                                GENERIC_WRITE, // Write access
                                FILE_SHARE_READ | FILE_SHARE_WRITE, // Share mode
                                NULL, // Default security
                                OPEN_EXISTING, // Open the existing drive
                                FILE_FLAG_NO_BUFFERING | FILE_FLAG_WRITE_THROUGH, // Unbuffered write, bypass cache
                                NULL // No template file
    );

    if (hDrive == INVALID_HANDLE_VALUE) {
        mDebug() << "Failed to open drive: " << GetLastError();
    }

    BYTE buffer[4096] = {0}; // Buffer filled with zeroes
    DWORD bytesWritten = 0;

    // Write zeroes to the entire drive
    while (WriteFile(hDrive, buffer, sizeof(buffer), &bytesWritten, NULL) && bytesWritten > 0) {
        // Keep writing zeroes until the entire disk is zeroed out
    }

    CloseHandle(hDrive);
    mDebug() << "Drive zeroed successfully.";
    return true;
}
...
void WinDriveProvider::checkDrives()
{
    ...
    for (auto it = usbDeviceList.cbegin(); it != usbDeviceList.cend(); it++) {
        bool mountable = true;
        auto partitionList = m_diskManagement->getDevicePartitions(it.key());
        ...
        auto diskDrive = m_diskManagement->getDiskDriveInformation(it.key(), it.value());
